...
function updateProgress(step, total, message) {
    ...
    if (stepCounter) stepCounter.textContent = `${step}/${total}`;
    ...
    if (elapsedTime) {
        elapsedTime.textContent = `⏱️ ${elapsedMinutes.toString().padStart(2, '0')}:${elapsedSecondsRemainder.toString().padStart(2, '0')}`;
    }

    // Estimate remaining time
    if (step > 0 && percentage < 100) {
        const avgTimePerStep = elapsed / step;
        const remainingSteps = total - step;
        const estimatedRemaining = avgTimePerStep * remainingSteps;
        const estimatedSeconds = Math.floor(estimatedRemaining / 1000);
        const estimatedMinutes = Math.floor(estimatedSeconds / 60);
        const estimatedSecondsRemainder = estimatedSeconds % 60;

        if (timeEstimate) {
            timeEstimate.textContent = `⏱️ Estimativa: ${estimatedMinutes}:${estimatedSecondsRemainder.toString().padStart(2, '0')} restantes`;
        }
    }
    ...
    if (percentage >= 100) {
        ...
        if (timeEstimate) {
            timeEstimate.textContent = `✅ Concluído em ${elapsedMinutes}:${elapsedSecondsRemainder.toString().padStart(2, '0')}`;
        }
...
function downloadPDF() {
    ...
    if (!analysisId) {
        alert('Erro: ID da análise não encontrado');
        return;
    }
    ...
    fetch(`/api/download-pdf/${analysisId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Erro ao gerar PDF');
            }
        ...
        .then(blob => {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `relatorio_analise_${analysisId}.pdf`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
